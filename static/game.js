...
function showInstallPrompt() {
  ...
  installDiv.style.bottom = '20px';
  installDiv.style.right = '20px';
  installDiv.style.background = '#fff';
  installDiv.style.border = '1px solid #ccc';
  installDiv.style.padding = '10px';
  installDiv.style.borderRadius = '5px';
  installDiv.style.boxShadow = '0 2px 5px rgba(0,0,0,0.2)';
  installDiv.innerHTML = `
    <p>¿Quieres instalar PeonKinGame en tu pantalla de inicio?</p>
    <button id="install-button">Instalar</button>
    <button id="cancel-button">Ahora no</button>
  `;
...
window.addEventListener('appinstalled', () => {
  console.log('PeonKinGame fue instalado en la pantalla de inicio');
});
...
function updateTheme() {
    ...
    squares.forEach(square => {
        const isWhiteSquare = (parseInt(square.dataset.row) + parseInt(square.dataset.col)) % 2 === 0;
        square.classList.remove('white', 'black', 'white-brown', 'black-brown', 'white-green', 'black-green', 'white-blue', 'black-blue');
        square.classList.add(isWhiteSquare ? `white-${boardColor}` : `black-${boardColor}`);
        square.classList.remove('piece-standard', 'piece-merida', 'piece-alpha');
        square.classList.add(`piece-${pieceStyle}`);
    });
}
...
function renderBoard() {
    ...
    const pieceColors = {
        ...
        'gold-silver': { white: '#d4af37', black: '#c0c0c0' },
        'red-blue': { white: '#ff0000', black: '#0000ff' }
    };
    const squares = document.querySelectorAll('.square');
    squares.forEach(square => {
