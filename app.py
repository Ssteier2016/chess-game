...
def check_session():
    data = request.get_json()
    username = data.get('username')
    if username and sessions.get(request.sid) == username and session.get('username') == username:
        user_data = load_user_data(username)
        conn = sqlite3.connect(DATABASE_PATH)
        c = conn.cursor()
...
def recover_password():
    ...
    c.execute('INSERT OR REPLACE INTO recovery_codes (username, code, created_at) VALUES (?, ?, ?)', (username, code, created_at))
    conn.commit()
    conn.close()
    print(f"Código de recuperación para {username}: {code}")
...
def logout():
    ...
    if sid in sessions:
        ...
        socketio.emit('waitlist_update', {
            'players': [{'sid': s, 'username': info['username'], 'chosen_color': info['chosen_color']} for s, info in available_players.items()]
        })
...
def on_join(data):
    ...
    game_type = data.get('game_type', 'chess')  # 'chess' o 'checkers'
    ...
    if room not in games:
        if game_type == 'chess':
            ...
            board = None  # Placeholder para Damas
        games[room] = {
            'game_type': game_type,
            'chessboard': board,
            'turn': 'white',
    ...
    if len(players[room]) == 2:
        ...
        if game_type == 'chess':
            ...
            board_state = [['.' for _ in range(8)] for _ in range(8)]  # Placeholder para Damas
        ...
        with chess.engine.SimpleEngine.popen_uci(STOCKFISH_PATH) as engine:
            ...
            if board.is_check():
                ...
                if board.is_checkmate():
                    ...
                    user_data = load_user_data(ssessions[sid])
...
def on_move(data):
    ...
    if game['game_type'] == 'checkers':
        emit('error', {'message': 'Movimientos de Damas no implementados'}, room=room)
        return
...
def on_save_game(data):
    ...
    conn = sqlite3.connect(DATABASE_PATH)
    c = conn.cursor()
    fen = games[room]['chessboard'].fen() if games[room]['game_type'] == 'chess' else ''
    c.execute('INSERT INTO saved_games (username, room, game_name, fen, turn) VALUES (?, ?, ?, ?, ?)',
              (username, room, game_name, fen, games[room]['turn']))
    conn.commit()
    conn.close()
        ...
        if board:
            for square in chess.SQUARES:
                ...
                if piece:
                    ...
                    board_state[row][col] = piece.symbol()
        games[room] = {
            'game_type': 'chess' if fen else 'checkers',
            'chessboard': board,
            'turn': turn,
            'time_white': None,
            'time_black': None
